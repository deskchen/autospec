...
# Try to import libclang bindings
try:
    # Try libclang-py3 first (more compatible with newer libclang)
    from libclang import cindex
except ImportError:
    # Fallback to clang.cindex
    try:
        import clang.cindex as cindex
    except ImportError:
        raise ImportError("Neither libclang-py3 nor clang package found. Please install one of them.")

def find_libclang():
    """
    Dynamically find libclang library path using multiple methods.
    Returns the path if found, None otherwise.
    """
    # Method 1: Use llvm-config if available (most reliable)
    try:
        # Try llvm-config-18 first (matches our LLVM 18 installation)
        for llvm_config in ['llvm-config-18', 'llvm-config']:
            try:
                ...
                # Look for libclang in the lib directory
                possible_paths = [
                    os.path.join(libdir, 'libclang.so'),
                    os.path.join(libdir, 'libclang.so.1'),
                    os.path.join(libdir, 'libclang-18.so.18'),
                    os.path.join(libdir, 'libclang-18.so'),
                ]
                ...
                x86_64_dir = os.path.join(libdir, 'x86_64-linux-gnu')
                if os.path.isdir(x86_64_dir):
                    for path in possible_paths:
                        alt_path = path.replace(libdir, x86_64_dir)
    ...
    if 'LIBCLANG_PATH' in os.environ:
        env_path = os.environ['LIBCLANG_PATH']
...
if libclang_path:
    ...
    print(f"Using libclang at: {libclang_path}", file=sys.stderr)
else:
    # Let the bindings try to find it automatically
    # This will use the default search paths
    print("Warning: Could not find libclang, using default search paths", file=sys.stderr)
    ...
    def _scan_children(self, parent_cursor, parent_node):
        ...
        for child in self._safe_get_children(parent_cursor):
            try:
                ...
                elif child.kind in (cindex.CursorKind.FOR_STMT, 
